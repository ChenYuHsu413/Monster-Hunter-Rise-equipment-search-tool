...
- secret 機制：`skill_base.csv` 有 `secret` 欄，**12 個技能** secret=2（Slugger/Agitator/Maximum Might/Latent Power/Heroics/Artillery/Free Meal/Bombardier/Divine Blessing/Slinger Capacity/Tool Specialist/Stamina Thief）。推測語意為「對應 Secret set bonus 生效時上限 +2」（如 挑戰者 5→7）。**此為推測，Phase 2 需對 Kiranico 抽驗證實** delta 是否恆為 +2、以及與 set-bonus「X Secret」技能名的連結方式。資料足以機械推導，符合 PLAN「不得硬編」要求。
