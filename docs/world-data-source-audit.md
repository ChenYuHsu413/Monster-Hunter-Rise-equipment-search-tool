...
### 斬味近似（v1 已知限制）— ⚠️ Phase 4 必辦交接
- MHWorldData `weapon_sharpness` **每把近戰武器僅一列**（`maxed` FALSE=2393 / TRUE=432；無法同時給匠0/匠5）。v1 取 `base = max = 該列值`，**不建模匠 +50 延伸**（World EFR 中 匠 不改變活性斬味色）。
- **交接 Phase 4（接 efr-world 前必辦）**：斬味 base=max 近似會使**匠在 EFR 中無效**，而 Fatalis 等 meta 武器依賴**匠 5 出紫斬**。Phase 4 必須先考證 `weapon_sharpness.csv` 該列語意（`maxed=FALSE` 到底是匠0 還是匠5 色帶、TRUE 又是哪端），據以**推導另一端**（匠0↔匠5 差 +50，分佈到色帶尾端），並**抽 3–5 把武器對 Kiranico World 武器詳細頁核對**斬味色帶後，才接上 EFR。未驗證前不得對 World EFR 排序下結論。
