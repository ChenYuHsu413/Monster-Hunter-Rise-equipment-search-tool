...
## 5. 斬味/匠與本模型的互動：期望斬味倍率（尾巴：紫斬長度模型）
接 world-sharpness-audit。**本節取代原「最高填色 color-only」寫法**（見下方「舊限制與推翻」）。

### 5.1 模型（`expectedSharp`）
- **色帶決定（同舊，不變）**：在 base(匠0)↔max(匠5) 間**依總長插值**求 `reach`，再以 max 色帶
  形狀截斷得目前各色段長度 `seg[i]`（低色先填、高色最後）。匠愈高 → reach 愈大 → 高色段愈長。
- **期望斬味倍率（新）**：不再只取頂端色的乘數，而是自色帶**頂端（最高色）往下取
  `EXPECTED_SHARPNESS_USE` 單位**，依落入各色段的長度**加權平均** raw/element 倍率：
  `sharpMult = Σ(取到的段長 × 該色乘數) / 取到的總長`。element 同法（`elem`）。
- `sharpColor` 仍回傳 tip 色（最高填色段），僅供顯示；`sharpMult` 改為上述期望值。

### 5.2 常數 `EXPECTED_SHARPNESS_USE`（預設 60，可調校準常數）
- **語意**：一輪輸出視窗（一段不中斷連段／一次磨刀間隔）約消耗的斬味單位數，用作加權視窗寬度。
- **依據**：CSV 斬味尺度下（Fatalis 紅 160、滿帶 ~400；1px=4 單位）多數近戰每命中約耗 1 單位，
  一段主力連段約 30–60 命中。取 **60** 使「頂端主色主導、但薄層尖端仍與其下一色顯著混合」——
  正是讓匠在物理 EFR 上重新可見的關鍵寬度。**非遊戲硬常數**；調大→更偏長期平均（薄紫貢獻被稀釋）、
  調小→更偏尖端瞬時（趨近舊 color-only）。同武器種內為排序指標，絕對值不重要，故此常數只影響
  「匠的邊際收益曲線陡度」，不影響跨武器種比較。
- **piecewise-linear 行為（實測 Fatalis 匠0→5）**：每級匠使頂端紫段 +10、視窗底端擠出等量的
  下一色。故單調上升，但**每級升幅取決於當級被擠出的是哪一色**：藍被擠出時每級 Δraw≈10×(1.39−1.20)/60，
  白被擠出時 Δraw≈10×(1.39−1.32)/60（較小）。Fatalis 匠0→5：2434→2619 EFR raw（+7.61%），
  乘數 1.2917→1.3900，逐級 +0.0317/+0.0317/+0.0117/+0.0117/+0.0117（前兩級擠藍、後三級擠白）。

### 5.3 仍未建模（誠實邊界）
- **剃刀銳利／砥石類技能不建模**：這些技能改變的是**磨刀速度／斬味回復量／耗損速率**（跨視窗的
  補充頻率），而非某一輪 60 單位視窗內的色段構成，屬另一層 uptime 近似，本模型不涉。利刃 World 無。
- **不建模視窗內耗損造成的色帶下滑**：`EXPECTED_SHARPNESS_USE` 是「取多長來平均」，非模擬打鬥中
  斬味實際遞減；視窗內各段以其**當前**色乘數計，不模擬命中順序造成的色階衰減。此為刻意簡化。
- 屬性上限（elemental cap）等其他近似見第 2 節，未受本次改動影響。

### 5.4 舊限制與推翻（逐條）
- 舊 `activeSharpIndex` 以「最高填色」為單一生效色。對 **base 已達最高色**的武器（Fatalis：base 已有
  薄紫 10），匠0/匠5 tip 同為紫 → 物理 EFR **匠-不變**，無法反映「匠使紫斬增厚」的真實收益。
- 本尾巴以期望斬味倍率**推翻此限制**：Fatalis 紫段 10→60，視窗內紫佔比隨匠上升，EFR 單調增。
  `smoke-efr-integration.mjs`(1) 的斷言已由「匠0/匠5 raw 相等」翻為「匠 0→5 raw 嚴格單調上升」。
- 副作用（合理且更準）：**tip 段 < 60 單位**的武器（薄白疊藍等），期望倍率**低於**舊「純 tip 色」值
  （薄尖端被其下一色稀釋）。例：Buster 匠0 tip 黃但期望 0.9583（<純黃 1.0）、匠5 tip 綠但期望 1.0167
  （<純綠 1.05）。排序 sanity（`smoke-efr-integration.mjs` 3a/3b）未變序：Fatalis 仍高於非畢業 GS。

## 6. 自測
- `scripts/world/test-efr-world.mjs`：5 組手算範例（純物理無斬味 / 屬性+期望斬味「短帶」 /
  條件技+secret / 期望斬味「厚白無紫」/ 期望斬味「薄紫」Fatalis 形狀），60 單位加權手算過程寫在
  腳本註解，函式輸出逐項一致（23/23）。
- `scripts/world/smoke-efr-integration.mjs`：(1) Fatalis 匠0→5 期望斬味 EFR 對照＋單調上升斷言、
  (2) Buster 匠改 tip 色、(3a/3b) Fatalis vs 非畢業 GS 排序 sanity（7/7）。
