...
- **信心度三層**:`confirmed`（TU/後期魔物的 MR 門檻,Game8 攻略確認的遊戲常數,3.3%）/ `inferred`（任務星級推導,57.8%）/ `unverified`（無來源魔物可推導,退回 rarity 近似,38.9%）。變種/傀異克服裝備另以名稱覆寫表修正來源誤判（社群回報機制,見 DATA-COVERAGE）。覆蓋率細節與待補清單見 [docs/DATA-COVERAGE.md](docs/DATA-COVERAGE.md)
