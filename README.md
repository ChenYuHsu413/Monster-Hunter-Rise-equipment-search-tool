...
網路上的配裝文九成是畢業裝,可是剛入坑的玩家根本一件都做不出來。本工具的定位是**進度導向**——不是又一個畢業配裝模擬器,而是回答新手最痛的問題:「**我現在做得出什麼?接下來該做哪一套?**」

![新手引導：輸入進度 → 現在最佳配裝 + 下一套目標](docs/guide-demo.gif)

- **新手引導模式（`/guide`）**:輸入你的遊戲進度（村莊★／集會所★／MR 章節／MR 等級,不用查攻略也答得出來）,直接得到「你現在的最佳配裝」與「下一套目標」——還缺哪些裝備、各要打哪隻魔物、解放條件為何,附 Kiranico 素材連結。
...
- 想要完整控制（固定部位、護石、保留洞位、傀異鍊成）的進階玩家,主頁的進階搜尋全部保留。

## 功能

- 武器系統:固定指定武器,或讓系統從同類型武器中搜尋;武器的洞數/攻擊/會心/屬性/自帶技能皆納入計算;全 14 種武器類型均為 Kiranico 真實資料
- 武器流派 preset(68 套,**全 14 武器類型 × 四階段全覆蓋**),依進度分四階段:初心(剛進大師位 MR1~3)/拓荒(MR 前期)/進階(中期)/畢業(TU5 終盤 meta),選取自動帶入推薦技能。**全 14 武器的畢業 preset 皆對齊 Game8 破曉 meta**(依各武器實際流派設物理/屬性向,如笛/斬斧/盾斧/弓為屬性流、大錘爆破、重弩擴散等)並標註 `Game8` 標籤;且**初心→拓荒→進階→畢業四階同一流派漸進**——早期是該武器 Game8 畢業 build 的輕量前置,逐階加深招牌技能,新手從初心就走在通往畢業的路上。太刀/雙刀另含舒適向與第二傷害變體。流派選擇為**依階段分組的下拉選單**,選中即顯示描述與標籤
- 分階評分權重:每個 preset 依 tier 帶入傷害/舒適/彈性權重(`TIER_SCORE_PROFILE`,畢業預設傷害向、初心舒適向),preset 可用 `scoreProfile` 覆寫(如「畢業舒適」)
- 階段限裝(取得門檻代理):初心/拓荒依 rarity 上限同時限制防具池與 search 武器候選(初心 ≤8 排除天迴龍等 MR5+ 裝、拓荒 ≤9、進階/畢業不限),讓早期階段實際搜到早期裝備與武器;固定部位/武器不受限
- **進度精確篩選（解放條件資料層）**:搜尋引擎另支援以玩家進度（村★/集會所★/MR 章節/MR 等級）精確過濾候選池(`BuildSearchRequest.progress` + `unlocks.json`,旗標疊加,未啟用時行為與 rarity 限裝完全相同);引導模式即以此運作,能抓到 rarity 近似會「白送」的裝備（如 R8 但實際 M4★ 才解放的花瓣X 系列）
- **新手引導模式（`/guide`）**:極簡進度輸入（localStorage 持久化）→「你現在的最佳配裝」（進度篩選 + 對應階段 preset,必要技能湊不滿時自動給近似解並標註）＋「下一套目標」（同流派下一階最佳解,逐件標記可做/還缺、解放條件與信心度、要打的魔物、Kiranico 素材連結）;未進 MR 者的目標為進入破曉後的第一套（初心）
...
- **信心度三層**:`confirmed`（TU/後期魔物的 MR 門檻,Game8 攻略確認的遊戲常數,3.6%）/ `inferred`（任務星級推導 + 傀異素材門檻,67.9%）/ `unverified`（無可推導依據,退回 rarity 近似,28.5%）。變種/傀異克服裝備以名稱覆寫表修正來源誤判;R9+ 裝備逐件掃描生產/強化素材,傀異素材依 Game8 等級表（A1★=MR10 ~ A4★=MR50、A5★+ 標註研究等級）設門檻。細節與待補清單見 [docs/DATA-COVERAGE.md](docs/DATA-COVERAGE.md)
...
- preset 覆蓋：全 14 武器 × 四階段已補齊（68 套），**畢業皆對齊 Game8 破曉 meta、四階同流派漸進**。可再擴：各武器的舒適向變體、多屬性變體，或同一武器的複數 meta 流派並列。
...
- 解放條件精確化：里程碑粒度已完成（`unlocks.json`，見下節）；可再擴：inferred 條目的人工校驗升級 confirmed（優先 68 套 preset 實際用到的裝備）、特殊解放套裝（活動/支線）的逐件標註。
...
流派 preset 的畢業 meta 配裝（技能取向）參考自 [Game8 — Monster Hunter Rise: Sunbreak Builds](https://game8.co/games/Monster-Hunter-Rise)，並以本工具的搜尋意圖（必要/偏好/權重）重新詮釋、對齊 Kiranico 官方譯名；標註 `Game8` 標籤者即為其對應流派。
