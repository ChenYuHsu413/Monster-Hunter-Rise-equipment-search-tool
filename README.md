...
- 武器流派 preset(68 套,**全 14 武器類型 × 四階段全覆蓋**),依進度分四階段:初心(剛進大師位 MR1~3)/拓荒(MR 前期)/進階(中期)/畢業(TU5 終盤 meta),選取自動帶入推薦技能。**畢業拆多取向**:太刀/雙刀各有傷害向、舒適向、屬性向與第二傷害變體(太刀寒氣居合、雙刀挑戰連擊);對齊 Game8 破曉 meta 者標註 `Game8` 標籤。流派選擇為**依階段分組的下拉選單**,選中即顯示描述與標籤
...
- preset 覆蓋：全 14 武器 × 四階段已補齊（68 套）。太刀/雙刀已對齊 Game8 破曉 meta（含傷害/舒適/屬性多取向）；其餘 12 類目前為各階段物理主流派各一套，可再擴屬性變體或對齊 Game8。
