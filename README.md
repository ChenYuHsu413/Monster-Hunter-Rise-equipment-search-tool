...
- **信心度三層**:`confirmed`（遊戲常數 + 人工驗證,4.6%）/ `inferred`（任務星級推導 + 傀異素材門檻 + 素材補判來源,68.6%）/ `unverified`（無可推導依據,退回 rarity 近似,26.9%）。變種/傀異克服裝備以名稱覆寫表修正來源誤判;R9+ 裝備逐件掃描生產/強化素材,傀異素材依 Game8 等級表（A1★=MR10 ~ A4★=MR50、A5★+ 標註研究等級）設門檻。細節與待補清單見 [docs/DATA-COVERAGE.md](docs/DATA-COVERAGE.md)
